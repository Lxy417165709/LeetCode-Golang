...
func (ms *MinStack) Push(x int) {
	nextMinValue := INF
	if !ms.minValueStack.IsEmpty() {
		nextMinValue = min(nextMinValue, ms.minValueStack.GetTop())
	}
	nextMinValue = min(nextMinValue, x)
...
func (ms *MinStack) Push(x int) {
	nextMinValue := INF
	if !ms.compositeStack.IsEmpty() {
		nextMinValue = min(nextMinValue, ms.compositeStack.GetTop())
	}
	nextMinValue = min(nextMinValue, x)
